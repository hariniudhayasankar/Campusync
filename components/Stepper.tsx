...
function StepIndicator({ step, currentStep, onClickStep, disableStepIndicators }: StepIndicatorProps) {
  ...
  return (
    <motion.div onClick={handleClick} className="stepper-indicator" animate={status} initial={false}>
      <motion.div
        variants={{
          inactive: { scale: 1, backgroundColor: 'var(--bg-card-elevated)', color: 'var(--text-muted)', borderColor: 'var(--border-base)' },
          active: { scale: 1, backgroundColor: 'var(--accent-primary)', color: 'var(--accent-primary)', borderColor: 'var(--accent-primary)' },
          complete: { scale: 1, backgroundColor: 'var(--success)', color: 'var(--success)', borderColor: 'var(--success)' }
        }}
...
function StepConnector({ isComplete }: StepConnectorProps) {
  const lineVariants: Variants = {
    incomplete: { width: 0, backgroundColor: 'var(--border-base)' },
    complete: { width: '100%', backgroundColor: 'var(--accent-primary)' }
  };
